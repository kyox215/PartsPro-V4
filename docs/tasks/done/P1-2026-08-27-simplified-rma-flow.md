...
完成定义分为规划、实现和发布三层，三层均已达到本任务的收口标准：

1. 规划层：业务规则、数据契约、状态机、批准门、验收、发布和回滚边界已明确并记录。
2. 实现层：客户一页式 RMA、拍照/相册与附件校验、后台队列/动作工作台、状态/数量/退款/库存守卫、Migration A/B/ACL 以及相关测试已交付。
3. 发布层：最终组合提交 `3403a37` 已完成生产发布；PartsPro-V4 已应用三份 RMA migration，Vercel production deployment `dpl_FhzjA4wZQpZ9vAYvkneHUPgiZ43s` 以该提交 READY，`partspro.app` 与 `www.partspro.app` 匿名 smoke 通过。剩余限制与回滚提示见文末。
...
- 新 draft/ticket/complete/submit 路径最多 6 张图片；V1 新流程不支持视频；每张在客户端压缩后不超过 4MB，服务端仍需重新校验字节、MIME、魔数和 SHA-256。Migration A 保留历史 `rma-evidence` bucket 兼容性，随后由 Migration B 收紧新上传类型和大小并保留历史对象；legacy 双协议仅作为历史数据兼容边界。
...
- 交付页面范围：客户订单详情、`/account`、`/rma`；后台 RMA panel、收货/质检/库存处置工作区。
- 交付 API 范围：客户 RMA draft/upload/complete/submit、兼容 GET/旧 POST 边界、后台 RMA action v3/legacy wrapper、客户一键寄回和服务端工作台投影。
- 当前批次数据表/RPC：`rma_requests`、`rma_request_events`、`rma_drafts`、`rma_attachments`、`rma_action_executions`、Storage、库存 movement、钱包退款和通知兼容字段；最终应用链为 Migration A、Migration B 和 ACL 收口 migration。
- 文档：本任务卡、后续 ADR、政策确认、runbook、验收记录和风险登记。
- 外部系统：V1 不接第三方物流/邮件/WhatsApp；支付原路退款可行性、意大利法定政策和税务处理需在上线前确认。

## 非目标

- 历史批次 1 的范围仅为服务端契约/API 与 additive Migration A；客户/后台 UI、Migration B/ACL 收口和生产发布在后续批次完成。
...
- Migration A 负责表、约束、兼容 RPC 和关系字段；Migration B 完成新流程切换和 bucket 收紧，ACL migration `20260828095944` 完成 `anon`/`authenticated` 直接权限收口。三份 migration 已在最终发布门禁后应用到 PartsPro-V4，服务端继续通过受控 RPC/service-role 访问。
...
- `policy_scope = statutory_b2c_withdrawal` 独立允许法定 B2C 无理由撤回不填原因/照片；缺陷、损坏、发错等原因仍要求至少一张图，B2B 规则不合并。意大利政策/法务确认仍是业务持续风险；Migration B 撤权与生产应用已完成，证据见最终验证记录。
...
- 该阶段验证范围先聚焦代码和源契约，数据库并发 trigger 验证列入最终发布门禁；最终 linked migration、ACL 和发布证据见文末，早期阶段限制不再代表当前状态。
...
- Migration A close guard 与 JS 规则成对收紧；当时尚未完成 UI、ACL 和生产链路，wallet 状态关系/替换单状态的实际数据库验证随后在 linked 安全门完成。
...
- 新增 `tests/rma-upload-client.test.mjs`，实际覆盖六张上限、MIME/视频拒绝、4MB/HEIC 边界、SHA-256、FormData/header、complete、重试与补偿删除、opaque payload 和 legacy endpoint 排除；随后由 2b 接入 `rma-page.tsx` 客户 UI。浏览器 canvas/真实 Storage 签名上传的生产凭据 E2E 仍属于残余验证限制。
...
- UI contract 测试覆盖三块、query 参数、camera/gallery、无视频/legacy、一次提交、无确认 modal 和安全 DTO；生产 `partspro.app`/`www.partspro.app` 匿名 smoke 已通过，但真实客户凭据下的 canvas/HEIC、移动端相机、CORS signed PUT、网络离线重试及完整业务写入 E2E 仍未执行。
...
- 2c 测试实际使用内存 fetch mock 覆盖响应丢失只提交、5+1 图片恢复、DELETE 500 后零新 ticket、取消清理、双协议安全边界和 orderNumber 源契约；浏览器刷新后的 File 会话恢复、跨标签并发、真实 Storage/CORS 与移动端相机仍属于残余验证限制。
...
- 对缺少 `receivedAt`、完整收货数量或 QC 的历史记录 fail-closed，落到最早安全队列并隐藏下游动作；库存 quarantine/未处分提供 `choose_inventory_disposition` 伪推荐，终态处分且底层 close 合法时才推荐关闭。真实 RPC/RLS/并发验证已纳入最终 linked 安全门；生产业务凭据 E2E 的限制见残余风险。
...
### 批次 3 服务端工作台契约、客户一键寄回与 Migration B（2026-08-28）
...
- Migration B `supabase/migrations/20260828092050_rma_workflow_finalize.sql` 完成 legacy POST/evidence 写协议收口、浏览器 direct insert/update 撤销、授权读取、私有 evidence bucket 的 4MiB/JPG/PNG/WebP/HEIC/HEIF 新上传限制（不删历史对象），并对新增函数设置显式 search_path 与最小 grant；最终已在 linked PartsPro-V4 应用。
- 该阶段尚未接入客户/后台 UI，后续由批次 4 完成；当时的 33/33、12/12、定向 ESLint 与 diff-check 结果已由最终 157 项定向集合、全量 lint/tsc/build 复核覆盖。
...
- 新增 `tests/rma-admin-ui-contract.test.mjs` 并扩展 customer UI contract，覆盖队列/count honesty、服务端动作、退款/替换候选、完整数量、照片缩略图、一键寄回和无确认弹窗；真实客户/管理员凭据业务写入 E2E 不在本次最终 smoke 范围内，详见残余风险。
- 本阶段代码随后合入最终组合提交 `3403a37`；production migration、Vercel 发布和匿名 smoke 的最终证据见文末。

### 批次 4a 生产后 RMA ACL 收口（2026-08-28）

- `supabase/migrations/20260828095944_rma_acl_lockdown.sql` 仅撤销 `anon`/`authenticated` 对五张 RMA 表和 `rma_request_no_seq` 的直接权限，并撤销两个私有 zero-arg trigger function 对 `PUBLIC`/浏览器角色的 EXECUTE；不修改 RLS、policy、函数定义、service_role 权限或数据。已应用到 linked `yiuxrjqexlfjtxxrkqvi / PartsPro-V4`。
- 应用后 ACL 核验确认五张 RMA 表与序列的浏览器角色直接权限已撤销，两个私有函数不再对 `PUBLIC`/`anon`/`authenticated` 可执行；service_role 权限保留，相关业务表行数不变。migration ledger 与生产 smoke 结果见最终验证证据。
...
生产门已完成：linked 目标、migration 顺序、ACL/RLS/Storage 关键隔离、并发/幂等契约、lint/build 与匿名 smoke 均有证据。真实客户/管理员凭据业务写入 E2E 未执行，属于明确残余风险，不把匿名 smoke 误记为真实交易 E2E。

## 发布与回滚方案

- 已按 Migration A → Migration B → ACL 收口顺序完成 PartsPro-V4 应用，并以最终组合提交 `3403a37` 发布 Vercel production；production deployment `dpl_FhzjA4wZQpZ9vAYvkneHUPgiZ43s` 为 READY。
- 发布后保留历史 RMA 状态映射与既有对象；新图片策略按已冻结的原因/客户组规则执行，不强制历史申请补图。
- 若应用异常，先将 Vercel 回滚到上一份已验证 deployment 并保留已提交 RMA/事件；数据库权限只通过经批准的反向 migration 调整，不使用 `db reset`、批量删除或手工覆盖历史。
- 持续观察附件失败率、重复 RMA、状态卡住、退款待审批、库存差异、RLS 403/越权告警和通知失败率；观察项是运营后续动作，不改变本任务已完成状态。

## 禁止事项

- 生产 migration、代码发布和权限变更必须继续经过对应安全门；本任务已完成的应用证据不得被后续手工操作绕过。
- 不将匿名 smoke、静态测试或 dry-run 冒充真实客户/管理员交易 E2E；不在未经批准的范围内写入生产业务数据。
...
表中早期批次记录保留用于审计；其中的阶段性 `blocked`/`skipped`/未发布描述只代表当时门禁，最终发布收口以下列为准。
...
| 主仓库 `node_modules/.bin/tsc --noEmit --incremental false --project <worktree>/tsconfig.json` | historical blocked (superseded) | 1d 早期记录仅报既有 `src/app/api/admin/restock-requests/[id]/route.ts:17:12 RouteContext`；最终 `npx tsc --noEmit` 已通过 |
...
| `/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc --noEmit --incremental false --project <worktree>/tsconfig.json` | historical blocked (superseded) | 早期记录仅有既有 `src/app/api/admin/restock-requests/[id]/route.ts:17:12 RouteContext`；最终 `npx tsc --noEmit` 已通过 |
...
| `/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc --noEmit --incremental false --project <worktree>/tsconfig.json` | historical blocked (superseded) | 1f 早期记录仅报既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc --noEmit --incremental false --project <worktree>/tsconfig.json` | historical blocked (superseded) | 2a 早期记录仅剩既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc --noEmit --incremental false --project <worktree>/tsconfig.json` | historical blocked (superseded) | 2b 早期记录仅剩既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `'/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc' --noEmit --incremental false --project tsconfig.json` | historical blocked (superseded) | 2b 早期记录仅有既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `'/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc' --noEmit --incremental false --project tsconfig.json` | historical blocked (superseded) | 2c 早期记录仅剩既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
| `git diff --check` | passed | 2c 当前变更无空白错误 |
| `SUPABASE_TELEMETRY_DISABLED=1 DO_NOT_TRACK=1 supabase status` | historical blocked (superseded) | 早期本地环境记录：Docker daemon 不可用；最终已完成 linked migration apply、ACL post-check、build 与生产 smoke |
| Supabase linked/local lint、migration dry-run、build/E2E | historical skipped (superseded) | 早期批次范围记录；最终已完成 linked migration apply、migration ledger、lint/build 与生产 smoke，真实 authenticated business-write E2E 保留为明确残余风险 |
...
| `'/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc' --noEmit --incremental false --project tsconfig.json` | historical blocked (superseded) | 2d 早期记录仅有既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `'/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc' --noEmit --incremental false --project tsconfig.json` | historical blocked (superseded) | 2e 早期记录仅报既有 `src/app/api/admin/restock-requests/[id]/route.ts(17,12): Cannot find name 'RouteContext'`；最终 `npx tsc --noEmit` 已通过 |
...
| `'/Users/kyox215/Documents/partspro v4/node_modules/.bin/tsc' --noEmit --incremental false --project tsconfig.json` | historical blocked (superseded) | 批次 4 早期记录仅报既有 `src/app/api/admin/restock-requests/[id]/route.ts:17:12 RouteContext`；最终 `npx tsc --noEmit` 已通过 |
| `git diff --check` | passed | 批次 4 UI、i18n、测试和任务卡无空白错误 |
| RMA + supplier final targeted tests | passed | 最终定向集合 `157/157` 通过，覆盖 RMA 规则、API/SQL contract、上传/附件、客户/后台 UI 与 supplier 相关 contract |
| `npm run lint` | passed | 合并后全量 ESLint 通过 |
| `npx tsc --noEmit` | passed | 合并后 TypeScript 检查通过，无新增诊断 |
| `npm run build` | passed | Next.js production build 通过，最终组合提交可构建 |
| `git diff --check` | passed | 最终组合提交无空白错误 |
| PartsPro-V4 migration apply | passed | linked `yiuxrjqexlfjtxxrkqvi / PartsPro-V4` 已按 A → B → ACL 应用 `20260828092046`、`20260828092050`、`20260828095944`；migration ledger 对齐，业务表行数不变 |
| RMA ACL post-apply verification | passed | 五张 RMA 表与 `rma_request_no_seq` 对 `anon`/`authenticated` 的直接权限已撤销；两个私有 zero-arg trigger function 不再对 `PUBLIC`/浏览器角色可执行；`service_role` 权限保留 |
| Vercel production deployment | passed | deployment `dpl_FhzjA4wZQpZ9vAYvkneHUPgiZ43s` 对应 commit `3403a37`，状态 `READY` |
| production anonymous smoke | passed | `partspro.app` 与 `www.partspro.app` smoke 通过；匿名访问按预期保持登录/权限边界 |
| authenticated customer/admin write E2E | not run | 未使用真实客户/管理员凭据执行会产生生产业务数据的写入测试；不把匿名 smoke 作为真实交易 E2E 证据 |

## 执行记录

- 创建：2026-08-27
- 批准：2026-08-27；用户批准本轮实现、推送与部署，生产写入与发布均按对应安全门执行
- 开始：2026-08-27
- review：2026-08-28；RMA/权限/隐私/退款/库存专项审查完成，问题修正后通过最终组合验收
- verified：2026-08-28；最终定向集合 `157/157`、全量 lint、tsc、build、diff-check 通过；A/B/ACL migration ledger、ACL post-check 和 `partspro.app`/`www.partspro.app` smoke 通过
- released：2026-08-28；组合提交 `3403a37` 对应 Vercel production deployment `dpl_FhzjA4wZQpZ9vAYvkneHUPgiZ43s`，状态 `READY`
- migration_applied：2026-08-28；`20260828092046`、`20260828092050`、`20260828095944` 已应用到 `yiuxrjqexlfjtxxrkqvi / PartsPro-V4`，ACL 验证通过且业务表行数不变
- closed：2026-08-28；任务卡移入 `docs/tasks/done/`

## 结果

最终组合提交 `3403a37` 交付服务端 RMA 工作台契约、客户一键寄回、精确退款预览、replacement candidate 读取、Migration A/B/ACL、客户/后台工作台 UI，并已完成 PartsPro-V4 应用与 Vercel production 发布。`partspro.app`/`www.partspro.app` 匿名 smoke 通过；真实客户/管理员凭据业务写入 E2E 未执行，保留为明确残余风险。

## 残余风险与后续任务

- 意大利 B2C 撤回/保修、B2B 商业退货、退款方式、运费承担和税务处理尚未由专业人员确认。
- 当前历史 RMA 状态、附件 JSON、wallet `order_void` 语义和库存处置数据需要迁移前盘点。
- 当前批次已收口服务端附件上传/校验、legacy 双协议隔离、核心状态/QC 守卫、active membership/employee-self 归属、并发数量锁、统一订单行净可退数量、V1 完整数量闭环、customer DTO 隔离、商业互斥、双轴关闭、显式收货时间、approved wallet/replacement linkage close guard、批量 GC 函数和基础通知事件，并交付客户三块 UI、后台队列/动作 UI 与一键寄回 UI；历史无单价快照 RMA 仍需人工/受控迁移补建后才能安全退款。真实客户/管理员凭据下的生产业务写入 E2E 未执行；真实视觉、相机权限、Storage/CORS 和完整退款/税务确认仍需后续专项验证。
- V1 明确拒绝部分收货/部分退款/部分库存处置；未来若需拆分，必须新增数量 ledger、库存批次分配和逐行退款对账，不得只放宽当前 action 参数。
- Migration A/B/ACL 已按批准顺序应用并完成 ledger/ACL 复核；后续如修改权限或 Storage，仍需重新执行 migration 安全门，不得直接手工放权。
- 真实客户/管理员凭据业务写入 E2E 尚未执行；本次仅以静态/定向测试、生产匿名 smoke 和 migration/ACL post-check 作为发布证据，不能扩大解释为完整交易闭环验证。
