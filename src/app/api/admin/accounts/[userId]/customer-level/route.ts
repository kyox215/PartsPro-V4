...
export async function PATCH(request: NextRequest, { params }: AccountParams) {
  ...
  try {
    ...
    if (
      ...
      return apiError(502, "ADMIN_EMPLOYEE_SELF_CUSTOMER_REQUIRED", "Employee account level changes must target the self-purchase profile.", {
        userId: paramResult.data.userId,
      });
    }
