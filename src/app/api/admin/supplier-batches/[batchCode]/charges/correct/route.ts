...
import { correctAdminSupplierBatchChargeV2 } from "@/lib/partspro-repository";
import {
  toSupplierBatchCorrectionReceiptDto,
...
import { supplierBatchChargeV2CorrectSchema } from "../_schemas";
...
export async function POST(request: NextRequest, { params }: SupplierBatchChargeParams) {
  ...
  if (!hasSupplierBatchCostPermission(admin.authState, "correct")) {
    return apiError(403, "ADMIN_PERMISSION_DENIED", "Supplier batch correction permission is required.");
  }

  const body = await parseAdminJsonBody(request, supplierBatchChargeV2CorrectSchema);
  if (!body.ok) {
  ...
  try {
    const result = await correctAdminSupplierBatchChargeV2(decodedBatchCode, body.data);
    ...
    return NextResponse.json({
      data: toSupplierBatchCorrectionReceiptDto(result.data, requestContext),
    ...
    return repositoryErrorResponse(
      ...
      "Supplier batch cost correction could not be completed."
    );
  }
}
