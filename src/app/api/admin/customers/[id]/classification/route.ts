...
import { toAdminCustomerClassificationPatch, toAdminCustomerDto } from "../../_dto";
import { customerClassificationPatchSchema, customerIdParamSchema } from "../../_schemas";
...
export async function PATCH(request: NextRequest, { params }: CustomerParams) {
  ...
  const parsed = customerClassificationPatchSchema.safeParse(body.data);

  if (!parsed.success) {
    return apiError(
      400,
      "INVALID_ADMIN_CUSTOMER_CLASSIFICATION",
      "Customer classification payload is invalid.",
      { issues: formatZodIssues(parsed.error) }
    );
  }

  try {
    const result = await updateAdminCustomerClassification(
      paramResult.data.id,
      toAdminCustomerClassificationPatch(parsed.data)
    );
    ...
    return repositoryErrorResponse(
      ...
      "Customer classification could not be updated at this time."
    );
  }
}
