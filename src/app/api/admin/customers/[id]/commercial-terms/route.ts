...
import { customerCommercialTermsPatchSchema, customerIdParamSchema } from "../../_schemas";
...
export async function PATCH(request: NextRequest, { params }: CustomerParams) {
  ...
  const parsed = customerCommercialTermsPatchSchema.safeParse(body.data);

  if (!parsed.success) {
    return apiError(
      400,
      "INVALID_ADMIN_CUSTOMER_COMMERCIAL_TERMS",
      "Customer commercial terms payload is invalid.",
  ...
  try {
    ...
    return repositoryErrorResponse(
      error,
      "ADMIN_CUSTOMER_COMMERCIAL_TERMS_UPDATE_FAILED",
