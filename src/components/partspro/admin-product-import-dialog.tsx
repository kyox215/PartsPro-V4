...
export function AdminProductImportDialog({
  ...
  async function applyImport() {
    if (!sourceFile || !preview || !confirmed || preview.counts.blocked > 0) return;
    setPending("apply");
    setError(null);
    try {
      const form = new FormData();
      ...
      <DialogContent className="flex max-h-[94dvh] w-[calc(100vw-1rem)] max-w-6xl flex-col gap-0 overflow-hidden p-0 sm:w-[calc(100vw-2rem)]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          ...
          {result ? (
            ...
            <PreviewPanel preview={preview} copy={copy} />
          ) : (
            <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
              ...
              <Tabs value={mode} onValueChange={(value) => { setMode(value === "paste" ? "paste" : "file"); setPreview(null); setError(null); }}>
                ...
                <TabsContent value="paste" className="mt-4">
                  ...
                  <Textarea
                    ...
                    value={pasteText}
                    onChange={(event) => { setPasteText(event.target.value); setPreview(null); setError(null); }}
        ...
        <DialogFooter className="border-t border-slate-200 bg-white px-4 py-3 sm:px-6">
          {result ? (
            <>
              <Button variant="outline" onClick={() => { setPreview(null); setResult(null); setConfirmed(false); setError(null); }}>
            ...
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <label className={cn("flex items-start gap-2 text-sm", preview.counts.blocked > 0 && "text-slate-400")}>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <Button variant="outline" onClick={() => { setPreview(null); setConfirmed(false); setError(null); }} disabled={pending !== null}>
                  {copy.backToEdit}
                </Button>
                <Button onClick={() => void applyImport()} disabled={!confirmed || preview.counts.blocked > 0 || pending !== null}>
...
function PreviewPanel({ preview, copy }: { preview: ProductImportPreview; copy: typeof zhCopy }) {
  const cards = [
    ...
    <div className="space-y-4">
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-slate-200 md:block">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">{preview.rows.map((row) => <PreviewTableRow key={row.rowNumber} row={row} copy={copy} />)}</tbody>
        </table>
      </div>
      <div className="space-y-3 md:hidden">{preview.rows.map((row) => <PreviewMobileCard key={row.rowNumber} row={row} copy={copy} />)}</div>
    </div>
  );
}

function PreviewTableRow({ row, copy }: { row: PreviewRow; copy: typeof zhCopy }) {
  return <tr className="align-top"><td className="px-3 py-3 font-mono text-xs">{row.rowNumber}</td><td className="px-3 py-3"><StatusBadge status={row.status} copy={copy} /></td><td className="px-3 py-3">{operationLabel(row.operation, copy)}</td><td className="px-3 py-3 font-mono text-xs">{row.sku}</td><td className="px-3 py-3"><div className="max-w-xs font-semibold text-slate-900">{row.normalized.name || "—"}</div><div className="mt-1 text-xs text-slate-500">{[row.normalized.brand, row.normalized.category].filter(Boolean).join(" · ")}</div></td><td className="px-3 py-3 text-xs text-slate-600">{row.changes.length ? row.changes.slice(0, 3).map((change) => change.field).join("、") : "—"}{row.changes.length > 3 ? ` +${row.changes.length - 3}` : ""}</td><td className="max-w-sm px-3 py-3 text-xs"><IssueList row={row} /></td></tr>;
}

function PreviewMobileCard({ row, copy }: { row: PreviewRow; copy: typeof zhCopy }) {
  return <div className="rounded-xl border border-slate-200 p-3"><div className="flex items-start justify-between gap-2"><div><div className="font-semibold text-slate-950">{row.normalized.name || row.sku}</div><div className="mt-1 font-mono text-xs text-slate-500">#{row.rowNumber} · {row.sku}</div></div><StatusBadge status={row.status} copy={copy} /></div><div className="mt-3 flex flex-wrap gap-2"><Badge variant="outline">{operationLabel(row.operation, copy)}</Badge>{row.changes.length > 0 && <Badge variant="outline">{copy.changes} {row.changes.length}</Badge>}</div><div className="mt-3"><IssueList row={row} /></div></div>;
}

function IssueList({ row }: { row: PreviewRow }) {
  if (row.issues.length === 0 && row.warnings.length === 0) return <span className="text-emerald-700">✓</span>;
  return <div className="space-y-1">{row.issues.map((issue) => <div key={issue} className="flex gap-1 text-red-700"><XCircle className="mt-0.5 size-3 shrink-0" /><span>{issue}</span></div>)}{row.warnings.map((warning) => <div key={warning} className="flex gap-1 text-amber-700"><AlertTriangle className="mt-0.5 size-3 shrink-0" /><span>{warning}</span></div>)}</div>;
}
...
function MetricCard({ label, value, tone }: { label: string; value: number; tone: string }) {
  const tones: Record<string, string> = { slate: "bg-slate-50 text-slate-800", blue: "bg-blue-50 text-blue-800", violet: "bg-violet-50 text-violet-800", amber: "bg-amber-50 text-amber-800", emerald: "bg-emerald-50 text-emerald-800", red: "bg-red-50 text-red-800" };
...
const zhCopy = {
  ...
  uploadTab: "上传文件",
  pasteTab: "粘贴表格",
  fileLabel: "选择 Excel 或 CSV",
  fileHint: "支持 .xlsx、.csv",
...
const itCopy: typeof zhCopy = {
  ...
  fileLabel: "Seleziona Excel o CSV",
  fileHint: "Supporta .xlsx e .csv",
