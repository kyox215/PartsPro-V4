-- Align generated phone model series with Mobilax's mobile-phone series
-- navigation. Apple remains the only brand without a model_series cascade.
  ...
  select case
    ...
    when lower(btrim(_brand)) = 'realme' then
      case
        ...
        when btrim(_model) ~* '\m(c|gt|x|p)\s*\d' then 'Series C/GT/X/P'
        else 'Realme Other'
      end
