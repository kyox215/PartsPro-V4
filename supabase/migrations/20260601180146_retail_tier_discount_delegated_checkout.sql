...
begin
  ...
  if v_can_view then
    ...
    if v_customer_price.id is not null then
      ...
      v_price_source := case
        when v_customer_type = 'retail' and v_level_discount_percent > 0 then 'retail_customer_level'
...
comment on function private.resolve_customer_product_price(uuid, uuid, integer) is
  'Resolves PartsPro retail/wholesale customer prices. Retail uses retail_price with customer level discounts; wholesale uses b2b rules plus wholesale-only price groups/customer prices. price_version includes level for every customer type and excludes generic customers.updated_at.';
