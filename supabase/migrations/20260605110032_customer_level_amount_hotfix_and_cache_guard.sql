...
create or replace function private.is_customer_profile_complete_for_checkout(
  _company_name text,
  _email text,
  _phone text,
  _fiscal_code text,
...
immutable
as $$
  select nullif(coalesce(_company_name, ''), '') is not null
    and nullif(coalesce(_email, ''), '') is not null
    and nullif(coalesce(_phone, ''), '') is not null
    and nullif(coalesce(_fiscal_code, ''), '') is not null
    and nullif(coalesce(_billing_address, ''), '') is not null
    and nullif(coalesce(_shipping_address, ''), '') is not null
$$;

comment on function private.is_customer_profile_complete_for_checkout(text, text, text, text, text, text) is
  'Checks the current PartsPro customer profile requirements for price visibility and checkout. Contact name, VAT, PEC and SDI are optional.';
...
comment on function private.resolve_customer_product_price(uuid, uuid, integer) is
  'Hotfix resolver: customer levels apply fixed per-unit EUR amounts, wholesale price groups apply after level amounts, and customer product prices override both.';
