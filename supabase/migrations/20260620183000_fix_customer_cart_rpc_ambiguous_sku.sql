...
begin
  ...
  if exists (
    with parsed as (
      select
        upper(trim(coalesce(item.value ->> 'sku', ''))) as item_sku,
        case
    ...
    select 1
    from parsed as item
    where item.item_sku is null
       or item.item_sku = ''
       or item.item_sku !~ '^[A-Z0-9_+.-]{3,64}$'
       or item.item_quantity is null
       or item.item_quantity < 1
       or item.item_quantity > 999
  ) then
  ...
  if exists (
    with parsed as (
      select upper(trim(coalesce(item.value ->> 'sku', ''))) as item_sku
      from jsonb_array_elements(p_items) as item(value)
    )
    select 1
    from parsed as item
    group by item.item_sku
    having count(*) > 1
  ) then
  ...
  where cart.user_id = v_auth_uid
    and not exists (
      select 1
      from parsed as item
      where item.item_sku = cart.sku_code
    );

  with parsed as (
    select
      upper(trim(coalesce(item.value ->> 'sku', ''))) as item_sku,
      (item.value ->> 'quantity')::integer as item_quantity
    from jsonb_array_elements(p_items) as item(value)
  )
  ...
  select
    ...
    item.item_quantity
  from parsed as item
  on conflict on constraint customer_cart_items_user_sku_unique do update
