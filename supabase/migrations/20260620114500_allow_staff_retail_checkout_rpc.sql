...
begin
  ...
  if v_is_staff and p_customer_id is not null then
    ...
    if v_customer.status <> 'active'
      ...
      raise exception 'Delegated checkout requires an active assigned customer' using errcode = '42501';
    end if;
  end if;
...
comment on function public.create_order_transaction(jsonb, uuid, text, text, text, numeric, jsonb, numeric) is
  'Creates PartsPro orders with tax-included product prices; employee delegated checkout accepts active assigned retail or wholesale customers, and p_vat_rate is accepted for compatibility but ignored for new order VAT.';
