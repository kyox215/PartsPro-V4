-- Match Mobilax's catalog-directory granularity exactly for the imported
-- phone brands: Vivo and TCL are single-series directories, Samsung Xcover
-- uses Mobilax casing, Xiaomi Mix models live under Mi Mix / Max, and
-- Motorola ThinkPhone lives under Series One/C/Defy.
...
as $$
  select case
    ...
    when lower(btrim(_brand)) = 'xiaomi' then
      case
        when btrim(_model) ~* '^mix\M' then 'Mi Mix / Max'
    ...
    when lower(btrim(_brand)) = 'motorola' then
      case
        ...
        when btrim(_model) ~* '\mone\M'
          ...
          or btrim(_model) ~* '\mthinkphone\M'
          or btrim(_model) ~* '\mmoto\s+c(\M|\d)'
          or btrim(_model) ~* '^c\s*\d' then 'Series One/C/Defy'
        else 'Motorola Other'
      end
    when lower(btrim(_brand)) = 'vivo' then 'Vivo'
    when lower(btrim(_brand)) = 'tcl' then 'TCL'
