...
test("supplier batch mobile UI keeps advanced filters in an applied Sheet", () => {
  ...
  const mobileSheetStart = panelSource.indexOf("function SupplierBatchMobileFiltersSheet(");
  const mobileSheetEnd = panelSource.indexOf("\nfunction BatchMobileField", mobileSheetStart);
  assert.notEqual(mobileSheetStart, -1);
  assert.notEqual(mobileSheetEnd, -1);
  const mobileSheetSource = panelSource.slice(mobileSheetStart, mobileSheetEnd);
  const mobileSelectTriggers = mobileSheetSource.match(/<SelectTrigger id="supplier-batch-mobile-[^"]+" className="[^"]*min-h-11[^\"]*text-base"/g) ?? [];
  assert.equal(mobileSelectTriggers.length, 6, "all mobile filter selects must keep a 44px minimum trigger height");
  const mobileDescriptionStart = mobileSheetSource.indexOf("<SheetDescription");
  const mobileDescriptionEnd = mobileSheetSource.indexOf("</SheetDescription>", mobileDescriptionStart);
  assert.notEqual(mobileDescriptionStart, -1);
  assert.notEqual(mobileDescriptionEnd, -1);
  const mobileDescriptionSource = mobileSheetSource.slice(mobileDescriptionStart, mobileDescriptionEnd);
  assert.match(mobileDescriptionSource, /\{copy\.mobileFiltersDescription\}/);
  assert.doesNotMatch(mobileDescriptionSource, /formatAdminMessage\(copy\.filterCount/);
